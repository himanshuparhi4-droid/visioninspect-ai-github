...
def draw_wrapped_text(
    pdf: canvas.Canvas, text: str, x: float, y: float, max_width: float, font: str, size: int
) -> float:
...
def draw_image_if_exists(
    pdf: canvas.Canvas, image_path: str | None, x: float, y: float, width: float, height: float
) -> None:
...
def generate_inspection_report_pdf(inspection: Inspection) -> Path:
    ...
    if inspection.pass_fail == "Fail":
        ...
        badge_color = "#b76b05"
    draw_status_badge(
        pdf, inspection.pass_fail or "Pending", PAGE_WIDTH - MARGIN - 42 * mm, PAGE_HEIGHT - 22 * mm, badge_color
    )
    ...
    draw_label_value(
        pdf, "Defect Type", inspection.defect_type or "Unknown", MARGIN + col_width + 10 * mm, y, col_width
    )
    ...
    draw_label_value(
        pdf, "Severity Level", inspection.severity_level or "Pending", MARGIN + col_width + 10 * mm, y, col_width
    )

    y -= 18 * mm
    draw_label_value(pdf, "Product ID", inspection.product_id or "Unassigned", MARGIN, y, col_width)
    draw_label_value(
        pdf, "Batch Number", inspection.batch_number or "Unassigned", MARGIN + col_width + 10 * mm, y, col_width
    )
    ...
    draw_label_value(
        pdf,
        "Line / Shift",
        f"{inspection.production_line or 'Unassigned'} / {inspection.shift or 'Unassigned'}",
        MARGIN,
        y,
        col_width,
    )
    draw_label_value(
        pdf, "Source", inspection.source_label or inspection.source_type, MARGIN + col_width + 10 * mm, y, col_width
    )
    ...
    pdf.drawString(
        MARGIN,
        12 * mm,
        "Generated by VisionInspect AI - automated AI result requires production policy review before shipment decisions.",
    )
    ...
    draw_label_value(
        pdf,
        "Critical Location",
        "Yes" if explainability.get("critical_location") else "No",
        MARGIN + col_width + 10 * mm,
        y,
        col_width,
    )
    ...
    draw_label_value(
        pdf, "Reviewed At", inspection.reviewed_at or "Pending", MARGIN + col_width + 10 * mm, y, col_width
    )
